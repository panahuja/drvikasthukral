...
<?php 
$count=$db->num_rows("Select * from newscategory where cUrl='".$_REQUEST['url']."'"); 
if($count==0){$f->redirect(APP_URL);}

$getcat=$db->get_row("Select * from newscategory where cUrl='".$_REQUEST['url']."'");
$cattitle='Category - '.$getcat['cName'];
?>
...
<section>
  ...
  <?php
					$sql = "SELECT * FROM categoryvalue nv LEFT JOIN press p ON (nv.pid=p.pid) WHERE nv.cid = '".$getcat['cid']."' AND p.pStatus = 'Enable' AND pActive = 1 ORDER BY p.pDateTime DESC ";
					...
					$self=APP_URL.'category/'.$_GET['url'].'&';
					$Serial = 1;
					if($numrows != 0){
